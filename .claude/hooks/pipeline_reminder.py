...
RULE = (
    "Edge-cascade routing rule: every artifact (code, git/CLI commands, scripts, "
    "configs, commit messages) goes through the Canvas pipeline FIRST "
    "(NPU-first -> draft -> gate -> bounded GPU repair -> win/lose logger LAST). "
    "Do not hand-write from-scratch code or commands that skip it.\n"
    "DECOMPOSE first: check for independent/dependent sub-tasks before routing.\n"
    "  Fan-out independent sub-tasks:\n"
    "    uv run python scripts/mesh_solve_canvas.py --topology budget_fanout "
    "\"sub1\" \"sub2\" \"sub3\"\n"
    "  Iterate dependent sub-tasks (sequential budget calls, carry context).\n"
    "  Hybrid: fan-out the independent parts, iterate the dependent tail.\n"
    "Route a single atomic task:\n"
    "  uv run python scripts/mesh_solve_canvas.py --topology budget \"<task>\"\n"
    "Git/CLI gate note: shell output caps (Python gate) -> Tier 3 executes. "
    "Still route it -- pipeline is consulted and logged.\n"
